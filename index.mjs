...
const SYMBOLS = JSON.parse(
  fs.readFileSync("./assests/nifty500.json", "utf-8")
);
...
async function main() {
  ...
  for (const symbol of SYMBOLS.symbolsNS) {
    try {
      ...
      liveTable.push({ symbol,name:details.name,trade:liveSignal.trade, status: liveSignal.status, sma44: liveSignal.latest?.sma44, close: liveSignal.latest?.close,open: liveSignal.latest?.open, dist: liveSignal.latest?.distFromSMA });
