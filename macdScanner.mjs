...
async function main() {
  ...
  console.log(`  Timeframe  : Daily candles  (Yahoo Finance 1d)`);
  ...
  for (let i = 0; i < symbols.length; i++) {
    ...
    process.stdout.write(`  ⏳ Fetching [${i + 1}/${symbols.length}] ${symbol} (daily)...\r`);
    ...
    try {
      ...
      if (result.signalFired) printResult(symbol, result, i + 1, symbols.length);
      allResults.push({ symbol, result });
